    ...
    while True:
        ...
        for entry in entries:
            ...
            for link in entry.findall(
                "atom:link", namespaces=ATOM
            ):  # Check links within entries first for next skiptoken
                if link.get("rel") == "next":
                    href = link.get("href")
                    if "skiptoken=" in href:
                        try:
                            current_skiptoken = int(
                                href.split("skiptoken=")[1].split("&")[0]
                            )
            ...
            if feed_next_link is not None:
                ...
                if "skiptoken=" in href:
                    try:
                        ...
                        next_link_found = True
                        print(
                            f"Found next skiptoken in feed level: {current_skiptoken}"
                        )
                    except ValueError:
                        pass
        ...
        if (
            len(all_docs) >= GLOBAL_BATCH_LIMIT
        ):  # Check limit again after processing entries
            break
...
def push_to_hf(docs: List[Dict[str, str]], repo_id: str) -> None:
    ...
    print(
        f"--- Preparing to push {len(docs)} documents to Hugging Face repo: {repo_id} ---"
    )
    api = HfApi()
